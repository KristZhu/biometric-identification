...
public class DAO {
		...
		try (Connection conn = DriverManager.getConnection(DB_URL);
			...
			while (rs.next()) {
				...
				Student student = new Student(
						...
						rs.getString(6), stringToMap(rs.getString(7)));
	...
	public static void insertStudent(Student student) {
		String sql = "INSERT INTO Students VALUES (?, " + "'" + student.getId() + "' , " + "'" + student.getName()
				...
				+ student.getGrade() + "' , " + "'" + student.getAllVisits() + "')";
		try (Connection conn = DriverManager.getConnection(DB_URL);
	...
	public static void updateStudent(Student student) {
		String sql = "UPDATE Students SET feature = ?, "
				...
				+ student.getMajor() + "' , " + "grade = '" + student.getGrade() + "' , " + "visit = '"
				+ student.getAllVisits() + "'" + "WHERE id = " + student.getId();
		try (Connection conn = DriverManager.getConnection(DB_URL);
				PreparedStatement stmt = conn.prepareStatement(sql)) {
			stmt.setBytes(1, student.getFeature());
			stmt.executeUpdate();
